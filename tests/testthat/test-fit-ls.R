...
test_that("the warm start is an optimisation, not a different estimator", {
  # The bootstrap skips Nelder-Mead because each resample starts from the
  # point estimate. That is only legitimate if it finds an equally good fit.
  #
  # Note what is compared, and what is not. The parameters cannot be: at this
  # follow-up `h2` is unidentified, and resamples put it anywhere from a few
  # hundred days to 1e10, all with the same residual sum of squares. The two
  # searches stop at different points on that flat ridge, which is a property
  # of the problem rather than a difference between them. What must agree is
  # the objective they reach and the duration they imply.
  ...
  both <- .with_seed(7L, lapply(seq_len(15), function(b) {
    take <- sample(ids, length(ids), replace = TRUE)
    dd <- obs[unlist(by_id[as.character(take)], use.names = FALSE), ]
    list(
      cold = .ls_fit_core(dd$time_days, log(dd$y_obs), "biphasic", fit$par),
      warm = .ls_fit_core(dd$time_days, log(dd$y_obs), "biphasic", fit$par,
                          warm = TRUE)
    )
  }))

  for (x in both) {
    expect_equal(x$warm$convergence, 0L)
    expect_equal(x$warm$ssr, x$cold$ssr, tolerance = 1e-4)
    expect_equal(
      tstar("biphasic", .ls_from_working(x$warm$theta, "biphasic"), 0.202),
      tstar("biphasic", .ls_from_working(x$cold$theta, "biphasic"), 0.202),
      tolerance = 1e-3
    )
  }

  # The whole point: fewer objective evaluations for the same answer. Stated
  # over the run, not per replicate -- a resample whose warm start does not
  # converge falls back to the full search and costs more than it saved. That
  # is the safety net doing its job, and it is rare enough not to matter.
  calls <- function(which) {
    sum(vapply(both, function(x) x[[which]]$counts[["function_calls"]],
               numeric(1)))
  }
  expect_lt(calls("warm"), calls("cold"))
})
